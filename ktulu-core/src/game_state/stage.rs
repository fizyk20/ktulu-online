...
const NIGHT_0_SEQUENCE: [Activity; 12] = [
    Activity::Character(CharacterType::Prostitute),
...
const NIGHT_SEQUENCE: [Activity; 27] = [
    Activity::Character(CharacterType::Sheriff),
    Activity::Character(CharacterType::Pastor),
        ...
        match self.time {
            Time::BeforeStart => {
                if self.activity != None {
                    panic!(
                        "Invalid game stage: time={:?}, activity={:?}",
            ...
            Time::Night(0) => {
                let activity_index = NIGHT_0_SEQUENCE
                    .iter()
            ...
            Time::Night(i) => {
                let activity_index = NIGHT_SEQUENCE
                    .iter()
                    .position(|&x| Some(x) == self.activity);
                if let Some(activity_index) = activity_index {
